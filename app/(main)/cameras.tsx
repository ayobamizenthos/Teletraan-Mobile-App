  ...
  return (
    <View style={[StyleSheet.absoluteFill, setupStyles.wrapper]}>
      {/* Backdrop stays on while the popup is open (kept outside the opacity
          fade so it's active the moment the popup mounts). iOS gets a native
          BlurView; Android gets a solid dark tint — dimezisBlurView leaks a
          white cast along the screen edges on some OEM builds, and a uniform
          tint reads identically across every device instead of "professional
          on Pixel, broken on ZTE". */}
