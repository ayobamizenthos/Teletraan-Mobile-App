...
import InsetGloss from '../../components/inset-gloss'
...
function SetupOverlay({ onDismiss }: { onDismiss: (viewId: ViewOptionId | null) => void }) {
  ...
  return (
    <View style={[StyleSheet.absoluteFill, setupStyles.wrapper]}>
      {/* Backdrop stays on while the popup is open (kept outside the opacity
          fade so it's active the moment the popup mounts). Blur on every
          platform; the backdrop wrapper is overflow:hidden so BlurView's
          kernel can't sample past the screen edges. */}
        ...
        <View style={styles.pageInner}>
          ...
          {!showSetupOverlay && !filterActive && (
            <View style={styles.searchBarOuter}>
              ...
              <GradientStrokeBox width={349} height={48} rx={10} gradient={LINEAR_BG3_STROKE} />
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  pageIndicator: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    // paddingVertical:0 — the pill is only 32px tall; padding:10 on all sides
    // left just 12px for the 14pt "1/3" text (~17px tall), so the digits got
    // clipped on higher-DPI Androids like Redmi. Horizontal padding preserved.
    paddingHorizontal: 10,
    paddingVertical: 0,
    gap: 32,
  },
...
const setupStyles = StyleSheet.create({
  ...
  backdropBlur: {
    ...
    ...Platform.select({
      ...
      android: { backgroundColor: 'rgba(0,0,0,0.55)' },
    }),
  },
  divider: {
    position: 'absolute',
