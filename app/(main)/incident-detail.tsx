    ...
    ;(async () => {
      try {
        ...
        if (cacheDir && localUri.startsWith('file://')) {
          ...
          try {
            await FileSystemLegacy.copyAsync({ from: localUri, to: target })
            if (!cancelled) cachedShareUriRef.current = target
            return
          } catch {
            // copy failed — fall back to the raw asset localUri.
          }
        }
  ...
  const shareIncidentClip = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web' && typeof navigator !== 'undefined') {
        ...
        if (!finalUri) {
          Alert.alert('Share failed', 'Could not prepare the media file.')
          return
        }
        // Ensure the file lives in cache with a clean .png filename so the
        // Android FileProvider serves it as an image to the share target.
        const cacheDir = FileSystemLegacy.cacheDirectory
        if (cacheDir && !finalUri.includes(cacheDir)) {
          try {
            const target = `${cacheDir}${fileName}`
            await FileSystemLegacy.copyAsync({ from: finalUri, to: target })
            finalUri = target
          } catch {
        ...
        try {
          ...
          Alert.alert('Share failed', String(err?.message ?? err ?? 'Unknown error'))
        }
      }
    } catch {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerFixed} pointerEvents="box-none">
        <View style={styles.header}>
          ...
          >
            <GradientStrokeBox width={40} height={40} rx={10} />
            <BackArrowIcon />
          </Pressable>
