...
import InsetGloss from './inset-gloss'
...
function DateField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <View style={styles.dateInput}>
      ...
      <GradientStrokeBox width={143} height={40} rx={10} />
    </View>
  )
}
...
export default function AlertFilterModal({ onClose }: AlertFilterModalProps) {
  ...
  return (
    <View style={styles.backdrop}>
      {/* iOS: native BlurView. Android: solid dark tint on the backdrop —
          dimezisBlurView leaks a white cast at screen edges on some OEM
          builds. Uniform tint reads identical across every device. */}
      {Platform.OS === 'ios' && (
        <BlurView intensity={10} tint="dark" style={StyleSheet.absoluteFill} pointerEvents="none" />
      )}
      {Platform.OS === 'web' && (
        <View style={[StyleSheet.absoluteFill, styles.backdropWebBlur]} pointerEvents="none" />
      )}
...
const styles = StyleSheet.create({
  // Full-screen overlay covering the parent (alerts) screen. Android uses a
  // heavier dark tint (no native BlurView on Android — dimezisBlurView left a
  // white cast at screen edges on some OEMs); iOS/web keep a lighter tint so
  // their real blur reads through it.
  backdrop: {
    position: 'absolute',
    top: 0,
    left: 0,
    ...
    alignItems: 'center',
    zIndex: 1000,
    backgroundColor: Platform.OS === 'android' ? 'rgba(0,0,0,0.55)' : 'rgba(0,0,0,0.4)',
    overflow: 'hidden',
  },
  backdropWebBlur: {
  ...
  viewBtn: {
    ...
    ...Platform.select({
      web: {
        boxShadow: 'inset 4px 4px 4px 0 rgba(188,183,183,1), 0 0 5px 0 rgba(163,163,163,0.5)',
      } as any,
      default: {
        shadowColor: '#A3A3A3',
