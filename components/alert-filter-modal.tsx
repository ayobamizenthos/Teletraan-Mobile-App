...
export default function AlertFilterModal({ onClose }: AlertFilterModalProps) {
  ...
  return (
    <View style={styles.backdrop}>
      {/* Backdrop blur — dimezisBlurView on Android now that the backdrop
          clips to the screen, so kernel sampling never reaches past the
          edges and the old white cast can't reappear. */}
      {Platform.OS === 'ios' && (
...
const styles = StyleSheet.create({
  // Full-screen overlay covering the parent (alerts) screen. overflow:hidden
  // clips the BlurView so its kernel never samples past the modal bounds —
  // that's what caused the white edge cast on Android before.
  backdrop: {
    position: 'absolute',
    top: 0,
    left: 0,
