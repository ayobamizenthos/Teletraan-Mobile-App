...
//
// Each auth frame in Figma is a solid #3D3D3D ellipse with a layer effect
// `filter: blur(Npx)` over a #121212 background.
//
// Web renders a real HTML <div> with the CSS filter the browser applies
// natively — same primitive Figma uses.
//
// Native renders a pre-baked PNG generated by scripts/gen-glow.mjs, which
// runs the identical Gaussian blur through sharp at build time. That
// bypasses react-native-svg's feGaussianBlur (which is unreliable on
// Android) and guarantees the two platforms show the same pixels.
...
interface EllipseSpec {
  ...
  blurRadius: number
  asset: number
  // Padded canvas dimensions of the pre-baked PNG so the halo isn't clipped.
  assetW: number
  assetH: number
}
  ...
  return (
    <View style={[StyleSheet.absoluteFill, styles.noTouch]}>
      {ellipses.map((e, i) => {
        ...
        // The PNG canvas is padded by ~3 sigmas around the ellipse. Position
        // the image so the ellipse centre lands on the Figma spec centre.
