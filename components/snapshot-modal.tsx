  ...
  const onDownload = async () => {
    ...
    if (Platform.OS === 'web' && typeof document !== 'undefined') {
      ...
      try {
        ...
        if (!sourceUri) throw new Error('no local uri')

        // Copy the bundled asset into cache with a clean filename + extension
        // so the gallery picks it up as an image with a sensible name.
        const cacheDir = FileSystemLegacy.cacheDirectory
        let fileUri = sourceUri
        if (cacheDir) {
          const target = `${cacheDir}${fileName}`
          await FileSystemLegacy.copyAsync({ from: sourceUri, to: target })
          fileUri = target
        }
        ...
        Alert.alert('Download failed', String(err?.message ?? err ?? 'Unknown error'))
      }
    }
  }
