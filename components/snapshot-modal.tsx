...
export default function SnapshotModal({
  ...
  const onDownload = async () => {
    ...
    if (Platform.OS === 'web' && typeof document !== 'undefined') {
      ...
      // Native: this is a Download button, not a share. Save the snapshot
      // directly into the device's Photos / Gallery via expo-media-library
      // so the user gets the file in their phone storage — no share sheet,
      // no extra steps, matches the web download experience.
      try {
        ...
        if (cacheDir && localUri.startsWith('file://')) {
          ...
          try {
            ...
            // copy failed — save the downloaded localUri as-is.
          }
        }
